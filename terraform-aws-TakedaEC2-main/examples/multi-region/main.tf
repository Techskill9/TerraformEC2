module "EC2" {
  ...
  instance_tier       = "app"
  root_volume_size    = 50
  #The Active Directory domain to join Windows Instances to(onetakeda.com by default). You can specify a different domain or leave this tag empty to not join a domain.
  asec_djo = ""
  #ami_id = "ami-005ec5f8c2e4d298b"
  #Always specify ami_version in order to avoid drift for later run after new AMI release
  ami_version     = "4"
  user_data       = <<-EOT
